...
达到 417 节点/561 案例；C2 增加 84 项有界生成程序对照与启动门禁，
当前 **501 节点/645 案例通过**，见 [C2 退出审计](c2-correctness-exit-audit.md)。
CPU 基线为 **1412 passed**，均零 skipped；uniformity 为 CPU 分析与可选审计，
不作为 GPU 同步验收。Race 使用默认 warn，未覆盖域有明确
Unknown 告警，不据此宣称所有案例无竞争；详见 [Race 启动验收](race-launch-audit.md)。
C2 的单 program 组合测试显式 race=off，只提供语义与 bounds 门禁证据。
