...
- 合并仅包含未提前 `return` 的前驱；单侧返回后使用另一侧的定义、类型和条件。
  两侧都返回时后续语句不可达；该规则同时适用于 runtime-if 和 static-if。
- 在存活前驱中**单侧**定义的变量：分支后视为可能未定义，使用点报
...
当前仅保留未修改、自赋值及写回初始同值字面量的不变量，其余值保守扩大；
出口保留零次执行时值不变的条件关系。循环体 assume 不成为出口的无条件事实。
`+=` 明确拒绝，请使用普通赋值。具体证明边界见 [数据流与解释器](dataflow-interpreter.md)。
